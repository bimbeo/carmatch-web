...
export default function TravelAssistant({ pageType, destinationSlug, destinationName, compact = false }: TravelAssistantProps) {
  ...
  return (
    <section className={`rounded-[2rem] border border-brand-100 bg-white shadow-sm ${compact ? 'p-5' : 'p-6 lg:p-8'}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-black uppercase tracking-[0.18em] text-brand-600">Trợ lý chuyến đi</p>
            <h2 className="text-2xl font-black text-slate-950">Hỏi nhanh trước khi đặt xe</h2>
          </div>
        </div>
        {mode ? (
      ...
      <div className="mt-5 max-h-[360px] space-y-3 overflow-y-auto rounded-3xl bg-slate-50 p-3">
        {messages.map((message, index) => (
          <div key={`${message.role}-${index}`} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`rounded-2xl px-4 py-3 text-sm font-semibold leading-6 ${
                message.role === 'user'
                  ? 'max-w-[78%] bg-brand-600 text-white'
                  : 'max-w-[92%] bg-white text-slate-700 shadow-sm ring-1 ring-slate-100 md:max-w-[74%]'
              }`}
            >
        ...
        {loading ? (
          <div className="flex justify-start">
            <div className="inline-flex items-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-500 shadow-sm ring-1 ring-slate-100">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {suggestedPrompts.map((prompt) => (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-brand-50 px-4 py-2 text-xs font-black text-brand-700 transition hover:bg-brand-100"
          >
      ...
      <div className="mt-4 flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold outline-none transition focus:border-brand-400 focus:ring-2 focus:ring-brand-100"
          placeholder="VD: 6 người đi Hạ Long 2 ngày nên chọn xe gì?"
        />
        <button
          type="button"
